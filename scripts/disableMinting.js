...
async function main() {
    ...
    if (!currentMintableState) {
        console.log("Minting is already disabled. Exiting...");
        return;
    }
    ...
    if (!adminRole) {
        console.error("You must be an admin to disable minting. Exiting...");
        return;
    }

    // disable minting
    console.log(`Disabling minting on contract: ${contractAddress}`);
    const tx = await bltnft.setMintable(false); // You can adjust this value as needed
    await tx.wait();

    console.log("Minting has been disabled successfully.");
}
